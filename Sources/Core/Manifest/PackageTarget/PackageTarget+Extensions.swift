...
extension PackageTarget: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let stringType = try? container.decode(String.self, forKey: .type) else {
            throw PackageTargetDecodingError.noTargetTypeSpecified
        }

        guard let name = try? container.decode(String.self, forKey: .name) else {
            throw PackageTargetDecodingError.noNameSpecified
        }
        ...
        case "binaryTarget":
            if let path = try? container.decode(String.self, forKey: .path) {
                ...
                throw PackageTargetDecodingError.invalidBinaryTarget
            }
        case "systemLibrary":
        ...
        case "target", "testTarget":
            let dependencies = try? container.decode([Target.TargetDependency].self, forKey: .dependencies)
            let path = try? container.decode(String.self, forKey: .path)
            ...
            if stringType == "target" {
                ...
                throw PackageTargetDecodingError.invalidTargetType
            }
...
extension PackageTarget {
    ...
    public enum PackageTargetDecodingError: Error {
        case noTargetTypeSpecified
