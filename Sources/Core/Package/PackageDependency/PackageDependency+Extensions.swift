...
extension PackageDependency: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let path = try? container.decode(String.self, forKey: .path) {
            self = .local(
                localDependency: .init(
                    path: path,
                    name: name
                )
            )
        } else if let url = try? container.decode(String.self, forKey: .url) {
            let requirement: RemoteDependency.Requirement
            ...
            if let from = try? container.decode(String.self, forKey: .from) {
                ...
                throw PackageDependencyDecodingError.invalidRemoteDependencyRequirement
            }

            self = .remote(
                remoteDependency: .init(
                    url: url,
            ...
            throw PackageDependencyDecodingError.invalidDependencyType
        }
    }
}

// MARK: PackageDependencyDecodingError

extension PackageDependency {

    public enum PackageDependencyDecodingError: Error {
        case invalidDependencyType
        case invalidRemoteDependencyRequirement
    }
}
